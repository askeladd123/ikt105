...
CREATE TABLE IF NOT EXISTS `mydb`.`customer_registration` (
  ...
  INDEX `fk_customer_has_branch_branch1_idx` (`branch_branch number` ASC) VISIBLE,
  INDEX `fk_customer_has_branch_customer1_idx` (`customer_member number` ASC) VISIBLE,
  CONSTRAINT `fk_customer_has_branch_customer1`
    FOREIGN KEY (`customer_member number`)
    REFERENCES `mydb`.`customer` (`member_number`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
